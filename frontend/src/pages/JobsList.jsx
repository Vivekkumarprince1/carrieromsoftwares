...
const Jobs = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || `Error deleting job with ID: ${id}`);
    }
  };
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    {currentUser && currentUser.role === 'user' && expandedJobId !== job._id && (
                      <button 
                        className="bg-gradient-to-r from-green-600 to-emerald-700 hover:from-green-500 hover:to-emerald-600 text-white px-4 py-2 rounded-lg transition duration-300 ease-in-out flex items-center gap-2 hover:shadow-md shadow-sm"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleApply(job);
                        }}
                  ...
                  <div className="border-t border-gray-700/50 p-6 space-y-6 bg-gradient-to-b from-gray-800/50 to-gray-900/80 animate-fade-in">
                    ...
                    {currentUser && (
                      <div className="pt-4 flex justify-center">
                        <button
                          className="bg-gradient-to-r from-green-600 to-emerald-700 hover:from-green-500 hover:to-emerald-600 text-white px-8 py-3 rounded-lg font-medium transform hover:scale-105 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-opacity-50 shadow-lg hover:shadow-yellow-600/20 flex items-center gap-2"
